...
class K:
    ...
    def get_lim(self,other):
        return int((other.k_pos+self.k_post)//2)
    ...
    '''def walk_dir(andares,left=None,right=None):
        ...
        self.n_dir-=self.andares[self.k_post+1]
        self.n_esq+=self.andares[self.k_post]
...
'''
    ...
    def simulate_movement_cost(self,c_first,n_first,n_second,c_second):
        c_first+=n_first # custo da direita = custo da direita - antigo numero de pessoas a direita
        n_first-=self.andares[self.k_post+direction]
        n_second+=self.andares[self.k_post]
    ...
    def get_lim_cost(self,andares,new_lim):
        return andares[new_left_lim]*(new_left_lim-self.k_pos),abs[andares[new_left_lim]]

    def get_dir_cost(self,andares,N,left=None,right=None):

        c_dir,n_dir,n_esq,c_esq=self.simulate_movement_cost(self.c_dir,\
        ...
        if right!=None:
            old_total_cost+=right.c_left
            new_right_lim=self.get_lim(right)

            if new_right_lim!=self.lim_right:
                ...
                right_left_cost=left.c_dir
                right_left_lim=left.lim_dir
                right_left_n=left.c_dir
    ...
    def walk_dir(self,andares,N,left=None,right=None):
        ...
        if left!=None:
            right.c_esq,right.n_esq,right.lim_esq=left_update
        return True
...
class Elevador_cost:
    ...
    def calc_solution(self):
        ...
        while in_loop:
            ...
            for i,k in (zip,self.solution_list):
